...
public:
   ...
   BOOL Free() {
      ...
      if (m_pulUserPfnArray != NULL) {
	     /*
			 ...
			 by using AllocateUserPhysicalPages or AllocateUserPhysicalPagesNuma. 
			 ...
			 This does not affect the virtual address space 
			 that is occupied by a specified Address Windowing Extensions (AWE) region. 
		  */
   ...
   BOOL MapStorage(CAddrWindow& aw) {
      /* Maps previously allocated physical memory pages 
	     at a specified address in an Address Windowing Extensions (AWE) region. */
