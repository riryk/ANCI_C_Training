...
CVMArray<TYPE>::CVMArray(DWORD dwReserveElements) {
   ...
   if (sm_pHead == NULL) {
      ...
      /* Enables an application to supersede the top-level 
	   * exception handler of each thread of a process.
       * After calling this function, if an exception occurs in a process 
	   * that is not being debugged, and the exception makes it to the unhandled exception filter, 
	   * that filter will call the exception filter function 
    ...
    *   without allocating any actual physical storage 
	*   in memory or in the paging file on disk. 
	* MEM_TOP_DOWN -
	*   Allocates memory at the highest possible address. 
	*   This can be slower than regular allocations, 
