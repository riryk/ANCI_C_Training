...
public:
   ...
   BOOL AllocBuffer(SIZE_T nBuffSize) 
   {
	   ...
	   *   the specified address is rounded down 
       ...
       *   If the lpAddress parameter is NULL, 
	   *   this value is rounded up to the next page boundary. 
	   *   Otherwise, the allocated pages include 
	   *   all pages containing one or more bytes in the range 
	   *   from lpAddress to lpAddress+dwSize. 
	   *   This means that a 2-byte range straddling a page boundary 
	   *   causes both pages to be included in the allocated region.
	   *
	   * MEM_COMMIT
	   *
	   * Allocates memory charges 
	   * (from the overall size of memory and the paging files on disk) 
	   ...
	   * the contents will be zero. Actual physical pages are not allocated unless/until 
       ...
       * An attempt to commit a page that is already committed 
...
BOOL FileCopy(PCTSTR pszFileSrc, PCTSTR pszFileDst) {
   ...
   try 
   {
	   ...
	   *  The template file supplies file attributes 
	   ...
	   *  the file inherits the discretionary access control list from its parent directory. 
	   *  For additional information, see File Encryption.
	   * 
	   */
      CEnsureCloseFile hFileDst = 
		  CreateFile(
		     pszFileDst, 
			 GENERIC_WRITE, 
             0, 
	  ...
	  /* 
	   * Sets the physical file size for the specified file 
